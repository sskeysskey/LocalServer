...
def init_analytics_db():
    ...
    #
    # baseline_*：
    #   用户最近一次观看/确认更新时，资源库的基准状态。
    #
    # last_catalog_*：
    #   服务器上一次扫描该剧时看到的资源库状态。
    #
    # watched_ordinal：
    #   用户目前看过的最高有效集数位置，用于追剧列表排序。
    c.execute('''
        CREATE TABLE IF NOT EXISTS video_follow_series (
            user_id TEXT NOT NULL,
            ...
            watched_ordinal INTEGER DEFAULT 0,

            baseline_effective_count INTEGER DEFAULT 0,
            baseline_s_season INTEGER DEFAULT 0,
            baseline_s_episode INTEGER DEFAULT 0,
            baseline_normal_count INTEGER DEFAULT 0,

            last_catalog_effective_count INTEGER DEFAULT 0,
            last_catalog_s_season INTEGER DEFAULT 0,
            last_catalog_s_episode INTEGER DEFAULT 0,
            last_catalog_normal_count INTEGER DEFAULT 0,
    ...
    c.execute('''
        CREATE INDEX IF NOT EXISTS idx_video_follow_user
        ON video_follow_series(user_id, is_cancelled)
    ''')

    # 每次“有效更新”产生一条按天记录。
    # API 只返回北京时间当天的 pending 提醒，因此昨天没点开的更新不会延续到今天。
    c.execute('''
        CREATE TABLE IF NOT EXISTS video_follow_notifications (
            user_id TEXT NOT NULL,
            series_url TEXT NOT NULL,
            ...
            previous_s_season INTEGER DEFAULT 0,
            previous_s_episode INTEGER DEFAULT 0,
            current_s_season INTEGER DEFAULT 0,
            current_s_episode INTEGER DEFAULT 0,

            pending INTEGER DEFAULT 1,
            badge_seen INTEGER DEFAULT 0,
            created_at TIMESTAMP NOT NULL,
            acknowledged_at TIMESTAMP,

            PRIMARY KEY (user_id, series_url, detected_date)
        )
    ''')

    c.execute('''
        CREATE INDEX IF NOT EXISTS idx_video_follow_notifications
        ON video_follow_notifications(user_id, detected_date, pending)
    ''')

    # 用户追剧界面偏好。
    # auto_popup=1：进入视频首页时主动弹出。
    # auto_popup=0：仅通过底部按钮手动打开。
    c.execute('''
        CREATE TABLE IF NOT EXISTS video_follow_preferences (
            user_id TEXT PRIMARY KEY,
            auto_popup INTEGER DEFAULT 1,
    ...
    # 给视频播放流水补充追剧所需字段。
    for ddl in [
        "ALTER TABLE event_logs ADD COLUMN series_url TEXT",
        "ALTER TABLE event_logs ADD COLUMN episode_name TEXT",
        "ALTER TABLE event_logs ADD COLUMN channel_name TEXT",
    ]:
        try:
            c.execute(ddl)
        except sqlite3.OperationalError:
...
def build_video_db():
    ...
    # 每次 JSON/播放源更新并重建 OVideo.db 后，自动扫描追剧更新。
    try:
        scan_video_follow_updates()
    except Exception as e:
        print(f"[OVideo Follow] OVideo.db 构建后扫描失败: {e}")
        traceback.print_exc()
...
_FOLLOW_CATEGORIES = {'Drama', 'Show', 'Anime'}

_EPISODE_SE_RE = re.compile(
    r'(?i)(?:^|[^A-Z0-9])S\s*0*(\d{1,3})\s*E\s*0*(\d{1,4})(?:[^0-9]|$)'
)
_EPISODE_DATE_RE = re.compile(r'(?<!\d)(20\d{6})(?!\d)')
_EPISODE_NUMBER_RE = re.compile(r'(\d{1,5})')
...
def _follow_clean_text(value):
    return re.sub(r'\s+', '', str(value or '')).lower()


def _follow_episode_part(name):
    text = str(name or '')
    for key, value in _PART_ORDER.items():
        if key in text:
            return value
    return 0


def _follow_normal_unit(name):
    """
    把普通集数名称归一化。

    第01集 / 第1集 / 国语01 / 粤语01 → n:1
    第1期上                         → n:1:p:1
    第1期下                         → n:1:p:3
    20241019                        → d:20241019
    HD / 正片                       → t:hd / t:正片
    ...
    cleaned = _follow_clean_text(text)

    date_match = _EPISODE_DATE_RE.search(cleaned)
    if date_match:
    ...
    number_match = _EPISODE_NUMBER_RE.search(cleaned)
    if number_match:
        number = int(number_match.group(1))
        part = _follow_episode_part(text)
        return {
            'key': f'n:{number}:p:{part}' if part else f'n:{number}',
            'sort': (1, number, part),
        }
...
def _follow_channel_metrics(channel, valid_urls):
    """
    返回单条线路的有效集数信息。

    有 SxxExx 时：
    - effective_count 统计所有季的有效集；
    - 更新前沿只看最新季的最大 E；
    - 往旧季补集不会触发“最新剧集更新”。
    ...
    - 日期型节目按日期分别计算。
    """
    episodes = channel.get('episodes') or {}

    playable = []
    for episode_name, episode_url in episodes.items():
        episode_url = episode_url or ''
        if episode_url in valid_urls or '.m3u8' in episode_url.lower():
            playable.append((str(episode_name), episode_url))
    ...
    parsed_s = []
    for original_index, (episode_name, episode_url) in enumerate(playable):
        match = _EPISODE_SE_RE.search(episode_name.upper())
        if match:
            parsed_s.append({
    ...
    # 只要该线路多数内容使用 SxxExx，就按多季结构处理。
    if parsed_s and len(parsed_s) >= max(1, len(playable) // 2):
        unique = {}
        for value in parsed_s:
            key = (value['season'], value['episode'])
            unique.setdefault(key, value)

        sorted_keys = sorted(unique.keys())
        url_ordinals = {}

        for ordinal, key in enumerate(sorted_keys, start=1):
            value = unique[key]
            url_ordinals[value['url']] = ordinal

        latest_season = max(key[0] for key in sorted_keys)
        latest_episode = max(
            key[1] for key in sorted_keys if key[0] == latest_season
        )

        return {
            'effective_count': len(sorted_keys),
            'normal_count': 0,
            's_season': latest_season,
            's_episode': latest_episode,
            'url_ordinals': url_ordinals,
            'has_s_format': True,
        }
    ...
    unique_units = {}
    for original_index, (episode_name, episode_url) in enumerate(playable):
        unit = _follow_normal_unit(episode_name)
        key = unit['key']

        # 国语01/粤语01等视为同一集，只保留第一次出现的线路位置。
    ...
    # 原始 JSON 顺序通常比纯数字排序可靠，例如“第1期上/第1期下”。
    ordered_units = sorted(
        unique_units.values(),
        key=lambda value: value['original_index']
    )

    url_ordinals = {}
    for ordinal, value in enumerate(ordered_units, start=1):
        url_ordinals[value['url']] = ordinal

    return {
        'effective_count': len(ordered_units),
        'normal_count': len(ordered_units),
        's_season': 0,
        's_episode': 0,
        'url_ordinals': url_ordinals,
        'has_s_format': False,
...
def _follow_catalog_metrics_from_row(row, episode_url=None):
    playlist = json.loads(row['playlist_json'] or '[]')
    valid_urls = _get_valid_urls()

    channel_metrics = [
        _follow_channel_metrics(channel, valid_urls)
        for channel in playlist
    ]
    channel_metrics = [
        value for value in channel_metrics
        if value['effective_count'] > 0
    ]
    ...
    effective_count = max(
        value['effective_count'] for value in channel_metrics
    )
    normal_count = max(
        (value['normal_count'] for value in channel_metrics),
        default=0
    )

    s_values = [
        (value['s_season'], value['s_episode'])
        for value in channel_metrics
        if value['has_s_format']
    ]
    s_season, s_episode = max(s_values, default=(0, 0))
    ...
    if episode_url:
        for value in channel_metrics:
            watched_ordinal = max(
                watched_ordinal,
                value['url_ordinals'].get(episode_url, 0)
            )
    ...
    return {
        ...
        's_season': s_season,
        's_episode': s_episode,
        'watched_ordinal': watched_ordinal,
    }


def _follow_get_catalog_metrics(series_url, episode_url=None):
    if not series_url or not os.path.exists(OVIDEO_DB_PATH):
        return None

    conn = _get_video_conn()
    try:
        row = conn.execute('''
            SELECT url, category, name, item_json, playlist_json
            FROM videos
            WHERE url=? AND category IN ('Drama', 'Show', 'Anime')
        ''', (series_url,)).fetchone()

        if not row:
            return None

        return _follow_catalog_metrics_from_row(row, episode_url)
    finally:
        conn.close()


def _follow_build_episode_lookup():
    """
    构建 episode_url -> series_url 映射。
    用于迁移旧版播放记录中没有 series_url 的数据。
    """
    result = {}
    ...
    try:
        rows = conn.execute('''
            SELECT url, playlist_json
            FROM videos
            WHERE category IN ('Drama', 'Show', 'Anime')
        ''').fetchall()

        for row in rows:
            playlist = json.loads(row['playlist_json'] or '[]')
            for channel in playlist:
                for episode_url in (channel.get('episodes') or {}).values():
                    if episode_url:
                        result[episode_url] = row['url']
    finally:
        conn.close()
...
def _follow_catalog_advanced(old_row, current):
    """
    判断是否发生了有效新增。

    1. SxxExx：
       最新季增加，或者最新季的 E 增加，才算有效新增。
       旧季补集不会仅因 effective_count 增加而触发。

    2. 普通线路：
       最大有效项目数量增加时触发。

    3. 如果同时存在 SxxExx 和普通线路：
       任意一类的有效前沿向前推进都可触发。
    """
    old_s = (
        int(old_row['last_catalog_s_season'] or 0),
        int(old_row['last_catalog_s_episode'] or 0),
    )
    new_s = (
        int(current['s_season'] or 0),
        int(current['s_episode'] or 0),
    )

    old_normal = int(old_row['last_catalog_normal_count'] or 0)
    new_normal = int(current['normal_count'] or 0)

    old_effective = int(old_row['last_catalog_effective_count'] or 0)
    new_effective = int(current['effective_count'] or 0)
    ...
    # 完全没有 SxxExx 数据时，以最大有效数作为最后兜底。
    if new_s == (0, 0) and old_s == (0, 0):
        return new_effective > old_effective

    return False


def _follow_record_watch(cursor, user_id, series_url,
                         episode_url, video_title=''):
    """
    用户实际播放某集后调用。
    ...
    已取消追剧：
      不自动恢复，保持黑名单状态。
    """
    if not user_id or not series_url:
        return

    current = _follow_get_catalog_metrics(
    ...
    existing = cursor.execute('''
        SELECT *
        FROM video_follow_series
        WHERE user_id=? AND series_url=?
    ''', (user_id, series_url)).fetchone()

    if existing is None:
        cursor.execute('''
            INSERT INTO video_follow_series (
                user_id, series_url, video_title, category, cover_image,
                watched_ordinal,

                baseline_effective_count,
                baseline_s_season,
                baseline_s_episode,
                baseline_normal_count,

                last_catalog_effective_count,
                last_catalog_s_season,
                last_catalog_s_episode,
                last_catalog_normal_count,
        ...
        ''', (
            user_id,
            series_url,
            current['video_title'] or video_title,
            current['category'],
            current['cover_image'],
            current['watched_ordinal'],
        ...
        # 已取消追剧的项目不因再次播放自动恢复。
        if int(existing['is_cancelled'] or 0) == 1:
            cursor.execute('''
                UPDATE video_follow_series
                SET last_watched_at=?,
                    watched_ordinal=MAX(watched_ordinal, ?)
                WHERE user_id=? AND series_url=?
            ''', (
                now,
                current['watched_ordinal'],
    ...
    # 用户已经实际播放该剧，表示当前更新已知晓。
    cursor.execute('''
        UPDATE video_follow_notifications
        SET pending=0,
            badge_seen=1,
            acknowledged_at=?
        WHERE user_id=? AND series_url=? AND pending=1
    ''', (now, user_id, series_url))


def seed_video_follow_from_history():
    """
    首次上线追剧功能时，把已有播放历史迁移为追剧基准。

    不会给旧历史立即制造更新提醒；
    只有以后 OVideo.db 再次出现有效新增时才会提醒。
    """
    if not os.path.exists(ANALYTICS_DB_PATH):
        return

    episode_lookup = _follow_build_episode_lookup()
    ...
    try:
        ...
        for row in rows:
            series_url = episode_lookup.get(row['video_url'])
            if not series_url:
                continue
            ...
            exists = c.execute('''
                ...
                FROM video_follow_series
                WHERE user_id=? AND series_url=?
            ''', (row['user_id'], series_url)).fetchone()
            ...
            current = _follow_get_catalog_metrics(
                series_url,
                episode_url=row['video_url']
            )
            if not current:
                continue

            watched_ordinal = current['watched_ordinal']

            c.execute('''
                INSERT OR IGNORE INTO video_follow_series (
                    user_id, series_url, video_title, category, cover_image,
                    watched_ordinal,
            ...
            ''', (
                row['user_id'],
                series_url,
                current['video_title'] or row['video_title'],
                current['category'],
                current['cover_image'],
                watched_ordinal,
        ...
        if inserted:
            print(f'[OVideo Follow] 从历史播放记录迁移了 {inserted} 条追剧基准。')

    except Exception as e:
        conn.rollback()
        print(f'[OVideo Follow] 历史追剧迁移失败: {e}')
        traceback.print_exc()
    finally:
        conn.close()


def scan_video_follow_updates():
    """
    在 OVideo.db 更新后扫描所有追剧项目。

    仅当本次资源库相较上次扫描发生有效前进时，
    才写入“今天”的更新提醒。
    """
    if not os.path.exists(ANALYTICS_DB_PATH):
        return
    ...
    seed_video_follow_from_history()

    today = today_str()
    now = now_iso()

    analytics = sqlite3.connect(
        ANALYTICS_DB_PATH,
        timeout=60.0,
    ...
    try:
        ...
        for follow in follows:
            video_row = video_rows.get(follow['series_url'])
            if not video_row:
                continue

            current = _follow_catalog_metrics_from_row(video_row)
            advanced = _follow_catalog_advanced(follow, current)

            if advanced:
                c.execute('''
                    INSERT INTO video_follow_notifications (
                        user_id,
                        series_url,
                        detected_date,
                    ...
                    VALUES (?,?,?,?,?,?,?,?,?,1,0,?)
                    ON CONFLICT(user_id, series_url, detected_date)
                    DO UPDATE SET
                        previous_count=excluded.previous_count,
                        current_count=excluded.current_count,
                        previous_s_season=excluded.previous_s_season,
                        previous_s_episode=excluded.previous_s_episode,
                        current_s_season=excluded.current_s_season,
                        current_s_episode=excluded.current_s_episode,
                        pending=1,
                        badge_seen=0,
                ...
                ''', (
                    ...
                    int(follow['last_catalog_effective_count'] or 0),
                    current['effective_count'],

                    int(follow['last_catalog_s_season'] or 0),
                    int(follow['last_catalog_s_episode'] or 0),
                    current['s_season'],
                    current['s_episode'],
        ...
        # 旧提醒只保留 30 天。
        cutoff = (
            datetime.now(APP_TZ) - timedelta(days=30)
        ).strftime('%Y-%m-%d')

        c.execute('''
            DELETE FROM video_follow_notifications
            WHERE detected_date < ?
        ''', (cutoff,))
        ...
        print(
            f'[OVideo Follow] 扫描完成：'
            f'{len(follows)} 个追剧项目，'
            f'{update_count} 条有效更新。'
        )
...
def _follow_resolve_series_url(series_url, episode_url):
    if series_url:
        return series_url
    ...
    lookup = _follow_build_episode_lookup()
    return lookup.get(episode_url)
...
def ovideo_search2():
    ...
    def classify(r):
        ...
        if name == q or name_n == qn:
            return 0, BIG
        if alias == q or alias_n == qn:
            return 1, BIG
        if director == q or director_n == qn:
            return 2, BIG
        cp = cast_exact_pos()
        if cp != BIG:
            return (3, cp) if cp <= 1 else (4, cp)

        # ---------- 第二大层：部分匹配（子串） ----------
        if q in name or qn in name_n:
            return 5, BIG
        if q in alias or qn in alias_n:
            return 6, BIG
        if q in director or qn in director_n:
            return 7, BIG
        cp = cast_contains_pos()
        if cp != BIG:
            return (8, cp) if cp <= 1 else (9, cp)
        if q in intro:
            return 10, BIG
        # 类型整词 / 其它子串
        return 11, BIG
    ...
    for r in exact_rows:
        ...
        results_map[r['url']] = {
            ...
            "mp": mp,
            "cast_pos": cast_pos,                 # 演员位次（同级最末级微调）
            "fuzzy": 1.0,
            "namelen": len(nm),                   # ⭐ 短名优先（全局生效）
            "rating": r['best_rating'] or 0.0,
            "rel": r['release_sort_key'] or "",   # ⭐ 上映日期（同级最主要）
    ...
    for s in scored[:limit]:
        if s['item_json']:
            item = json.loads(s['item_json'])

            category_name = next(
                (
                    key for key, value in cat_order.items()
                    if value == s['cat']
                ),
                None
            )
...
def track_event():
    try:
        ...
        # 老客户端没有传 series_url 时，尝试通过 episode_url 反查。
        series_url = _follow_resolve_series_url(
            series_url=series_url,
        ...
        # 只有实际播放才建立/更新追剧状态。
        # 下载完成不等同于用户已经观看。
        if event_type == 'play' and series_url:
            _follow_record_watch(
                cursor=c,
                user_id=user_id,
                series_url=series_url,
                episode_url=video_url,
...
@app.route('/api/OVideo/follow/status', methods=['GET'])
def video_follow_status():
    user_id = request.args.get('user_id')
    ...
    try:
        pref = c.execute('''
            SELECT auto_popup
            FROM video_follow_preferences
            WHERE user_id=?
        ''', (user_id,)).fetchone()

        auto_popup = True if pref is None else bool(pref['auto_popup'])
        ...
        rows = c.execute('''
            SELECT
                ...
                v.item_json,
                v.update_sort_key
            FROM video_follow_notifications n
            JOIN video_follow_series f
              ON f.user_id=n.user_id
             AND f.series_url=n.series_url
            JOIN videos v
              ON v.url=n.series_url
            WHERE n.user_id=?
              AND n.detected_date=?
              AND n.pending=1
              AND f.is_cancelled=0
              AND v.hide_blacklisted=0
        ...
        for row in rows:
            current_count = max(1, int(row['current_count'] or 0))
            watched_ordinal = max(0, int(row['watched_ordinal'] or 0))

            progress = min(
                1.0,
                watched_ordinal / current_count
            )
            ...
            result.append({
                ...
                'current_s_season': int(row['current_s_season'] or 0),
                'current_s_episode': int(row['current_s_episode'] or 0),
                'item': item,
            })

        # 观看进度比例越高越靠前；
        # 比例一致时按视频 update 日期倒序。
        result.sort(
            key=lambda value: (
                value['progress_percent'],
                value['item'].get('update') or ''
            ),
            reverse=True
        )
...
@app.route('/api/OVideo/follow/preference', methods=['POST'])
def video_follow_preference():
    data = request.get_json() or {}
    user_id = data.get('user_id')
    auto_popup = bool(data.get('auto_popup', True))
    ...
    try:
        conn.execute('''
            INSERT INTO video_follow_preferences (
                user_id,
                auto_popup,
                updated_at
            )
            ...
            ON CONFLICT(user_id)
            DO UPDATE SET
                auto_popup=excluded.auto_popup,
                updated_at=excluded.updated_at
        ''', (
            user_id,
            1 if auto_popup else 0,
            now_iso(),
        ))
        conn.commit()
        return jsonify({
...
@app.route('/api/OVideo/follow/mark_seen', methods=['POST'])
def video_follow_mark_seen():
    data = request.get_json() or {}
    user_id = data.get('user_id')
...
@app.route('/api/OVideo/follow/ack', methods=['POST'])
def video_follow_ack():
    data = request.get_json() or {}
    user_id = data.get('user_id')
    series_url = data.get('series_url')
    ...
    current = _follow_get_catalog_metrics(series_url)
    if not current:
...
@app.route('/api/OVideo/follow/cancel', methods=['POST'])
def video_follow_cancel():
    data = request.get_json() or {}
    user_id = data.get('user_id')
    series_url = data.get('series_url')
...
@app.route('/api/OVideo/follow/cancel_all', methods=['POST'])
def video_follow_cancel_all():
    data = request.get_json() or {}
    user_id = data.get('user_id')
    ...
    try:
        ...
        current_urls = [
            ...
            for row in c.execute('''
                SELECT series_url
                FROM video_follow_notifications
                WHERE user_id=?
                  AND detected_date=?
                  AND pending=1
            ''', (user_id, today)).fetchall()
        ...
        if current_urls:
            ...
            c.execute(f'''
                ...
                SET is_cancelled=1
                WHERE user_id=?
                  AND series_url IN ({placeholders})
            ''', [user_id] + current_urls)
            ...
            c.execute(f'''
                ...
                WHERE user_id=?
                  AND series_url IN ({placeholders})
            ''', [now_iso(), user_id] + current_urls)

        c.execute('COMMIT')

        return jsonify({
            'status': 'ok',
            'cancelled_count': len(current_urls),
        })
...
def admin_clear_db():
    ...
    try:
        ...
        if clear_type in ['analytics', 'all']:
            ...
            c.execute("DELETE FROM video_follow_series")
            c.execute("DELETE FROM video_follow_notifications")
            c.execute("DELETE FROM video_follow_preferences")
            conn.commit()
            conn.close()
...
if __name__ == '__main__':
    ...
    # 即使这次 OVideo.db 没有重建，也执行一次历史迁移和追剧扫描。
    scan_video_follow_updates()
